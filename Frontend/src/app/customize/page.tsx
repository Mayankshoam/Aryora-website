...
export default function CustomizePage() {
  ...
  const complete = styles.length > 0 && metal && shape && carat && contact.fullName && contact.emailId && contact.mobileNumber;
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const styleLabels = styles.map((id) => STYLES.find((s) => s.id === id)?.label).join(" + ");
      const summary = `Custom Ring Enquiry — Style: ${styleLabels}, Metal: ${METALS.find((m) => m.id === metal)?.label}, Diamond Shape: ${shape}, Carat Range: ${carat}.${notes ? " Notes: " + notes : ""}`;
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      ...
      <p className="text-ink/70 leading-relaxed mb-14 max-w-2xl">
        Choose one or more settings, a metal, and a diamond shape below. Our design concierge will
        follow up with a personalised quote — no two Aryora pieces are made the same way twice.
      </p>
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-12">
        <form onSubmit={submit} className="space-y-14">
          {/* Style — multi-select */}
          <div>
            <h2 className="font-display text-lg mb-1">1. Choose Setting Style(s)</h2>
            <p className="text-xs text-ink/50 mb-5">Select one or combine multiple — e.g. Halo + Vintage</p>
              ...
              {STYLES.map((s) => (
                <button
                  ...
                  className={`text-left border p-4 transition-colors ${
                    styles.includes(s.id) ? "border-emerald bg-emerald/5" : "border-black/10 hover:border-black/30"
                  }`}
                >
        ...
        <div className="lg:sticky lg:top-24 h-fit">
          ...
          <RingPreview metal={metal || "YELLOW_GOLD"} carat={carat || "0.5ct – 1ct"} />
        </div>
      </div>
    </div>
  );
